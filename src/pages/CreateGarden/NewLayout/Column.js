import { Button, IconButton, Input, Menu, MenuItem, Typography } from '@material-ui/core';
...
export default function Column({ index, width, handleVegetableDelete, handleVegetableForEditDisplayChange, handleVegetableWidthChange, fieldWidth }) {
    ...
    return (
        <>
            <div
                ...
                <div className={classes.widthEditDescription} style={{ display: `${editWidthMode ? '' : 'none'}` }}  >
                    <Input value={desiredWidth} onChange={(e) => setDesiredWidth(e.target.value)} style={{ maxWidth: 40 }}
                        inputRef={(button) => { inputRef = button }} />
                    <Typography variant="h7">cm</Typography>
                </div>
                <IconButton
                    onClick={() => handleVegetableWidthSubmit(index, desiredWidth)}
                    style={{ padding: 0, marginBottom: 10, display: `${editWidthMode ? '' : 'none'}` }}>
                    <DoneIcon />
                </IconButton>
                <div className={classes.widthDescription} style={{ display: `${editWidthMode ? 'none' : ''}` }}>
            ...
            <div className={classes.menu} >
                ...
                >
                    <MenuItem
                        onClick={() => handleVegetableChange()}>
                        Zmień warzywo
                    </MenuItem>
                    <MenuItem
                        onClick={() => handleEditWidthClick()}>
                        Zmień szerokość
                    </MenuItem>
                    <MenuItem
                        onClick={() => handleVegetableDelete(index)}>
                        Usuń warzywo
                    </MenuItem>
                </Menu>
            </div>
